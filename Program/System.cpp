...
void flush_user(istream& indev, user a[])
{
    ...
    {
        if (i > 0) indev.getline(a[i].username, 30);
        indev.getline(a[i].username, 30);
...
void flush_reader(istream& indev, reader a[])
{
    ...
    {
        indev >> a[i].code;
        if (i > 0) indev.getline(a[i].name, 30);
        indev.getline(a[i].name, 30);
        indev >> a[i].CID;
        indev >> a[i].birth.day;
...
void flush_book(istream& indev, book a[])
{
    ...
    while (!indev.eof())
    {
        if (i > 0) indev.getline(a[i].booktitle, 30);
        indev.getline(a[i].booktitle, 30);
        indev >> a[i].ISBN;
        ...
        {
            if (m == 0) indev.getline(a[i].genre[m], 20);
...
void create(user a[])
{
    ...
    cin.ignore();
    cout << "Nhap trang thai tai khoan" << endl;
